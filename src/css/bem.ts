import { classNames, type ClassNameValue, isRecord } from '@/css/classnames.js';
...
function applyMods(element: string, mod: ClassNameValue): string {
  ...
  if (isRecord(mod)) {
    return classNames(
      Object.entries(mod).map(([modifier, enabled]) => Boolean(enabled) && applyMods(element, modifier)),
    );
  }
